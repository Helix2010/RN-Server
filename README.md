...
为 RN-App 提供认证、移动端启动配置、版本/分发策略、Feature Flag、通用 API 和可观测性关联的服务端基座。默认形态是可拆分的模块化单体，优先保证契约稳定和运维可控，而不是预先拆微服务。
...
执行 `pnpm check` 可运行格式、Lint、类型、单元、E2E 与构建检查。

管理员密码只保存 scrypt 哈希。通过交互命令生成后写入 `.env` 的 `ADMIN_PASSWORD_HASH`：
